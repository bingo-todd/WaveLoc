...
class WavLoc(object):
    ...
    def _file_reader(self,record_set_dir):
        ...
        for sub_set_dir in dirs:
            fpath_all_sub = get_fpath(sub_set_dir,'.wav')
            fpath_all.extend([os.path.join(sub_set_dir,item) for item in fpath_all_sub])
    ...
    def _file_reader_noisy(self,record_set_dir):
        """ read wav files in given directies, one file per time
        Args:
            record_set_dir: directory or list of directories where recordings exist
        Returns:
        ...
        n_file = len(fpath_all)
        if len(fpath_all) <1:
            raise Exception('empty folder:{}'.format(record_set_dir))

        fpath_all_duplicate = fpath_all*n_SNR
        SNR_fpath_all = np.concatenate([np.ones(n_file)*SNRs[i]
                                        for i in range(SNRs)])
        # randomize fpath_all_duplicate and SNR_fpath_all
        rand_index = np.random.permutation(n_file*n_SNR)
        for i in range(n_file*n_SNR):
            fpath = fpath_all_duplicate[rand_index[i]]
            snr = SNR_fpath_all[rand_index[i]]
    ...
    def train_model(self):
        ...
        with self._graph.as_default():
            ...
            for epoch in range(last_epoch+1,self.max_epoch):
                ...
                self._add_log('epoch:{} lr:{} time:{:.2f}\n'.format(epoch,lr_value,iter_time))
